...
install_shortcut() {
  cat > /root/sbox/esperia.sh << EOF
#!/usr/bin/env bash
bash <(curl -fsSL https://github.com/vveg26/sing-box-reality-hysteria2/raw/main/beta.sh) \$1
EOF
  chmod +x /root/sbox/esperia.sh
  ln -sf /root/sbox/esperia.sh /usr/bin/esperia
...
uninstall_singbox() {
    ...
    rm -f /usr/bin/esperia
    rm -f /root/sbox/esperia.sh
...
# 作者介绍
print_with_delay "Reality Hysteria2 VmessArgo 三合一脚本 by Esperia" 0.03
echo ""
echo ""
#install pkgs
install_pkgs
# Check if reality.json, sing-box, and sing-box.service already exist
if [ -f "/root/sbox/sbconfig_server.json" ] && [ -f "/root/sbox/config" ] && [ -f "/root/sbox/esperia.sh" ] && [ -f "/usr/bin/esperia" ] && [ -f "/root/sbox/sing-box" ] && [ -f "/etc/systemd/system/sing-box.service" ]; then
    echo ""
